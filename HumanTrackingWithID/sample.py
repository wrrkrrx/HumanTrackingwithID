...
def main():
    ...
    while True:
        ...
        """
        Detect edilen uniq ID leri yukarıdan tags = at_detector.detect() kısmından alarak burada ayrıştırıyorum
        """
        ...
        print(manuelUniqList,uniq_List)
                         
        for trakcer_id, bbox in zip(t_ids, bboxes):
            if trakcer_id not in track_id_dict:   
                            
                #new_id = int([k for k in manuelUniqList if (v for v in uniq_List if k == v)])
                
    
                """
                Eşitlemeye çalıştım ama sürekli ID ye 1 atıyor. Amacım her yeni ID yi detect ettiğinde tracker_ıd ye onu atması
                Bu yüzden aşağıda bir eşitleme kurmaya çalıştım. İki listeyi karşılaştırıp Uniq ID yi alması için.
                Teşekkürler
                """
                
                new_id = int(all([x for x in uniq_List if x in manuelUniqList]))
